...
Clone the quarkusrobotshop-support project (in another directory)

```shell
git clone https://github.com/quarkusrobotshop/quarkusrobotshop-support.git
```

From inside the quarkusrobotshop-support project (on MacOS or Linux) run:
...
From the quarkusrobotshop-counter directory you can start the counter microservice with:
...
```shell
docker build -f src/main/docker/Dockerfile.native -t <<DOCKER_HUB_ID>>/quarkusrobotshop-counter .
```
```shell
export KAFKA_BOOTSTRAP_URLS=localhost:9092 \
PGSQL_URL="jdbc:postgresql://localhost:5432/robotshopdb?currentSchema=robotshop" \
PGSQL_USER="robotshopuser" \
PGSQL_PASS="redhat-21"
```

```shell
docker run -i --network="host" -e PGSQL_URL=${PGSQL_URL} -e PGSQL_USER=${PGSQL_USER} -e PGSQL_PASS=${PGSQL_PASS} e KAFKA_BOOTSTRAP_URLS=${KAFKA_BOOTSTRAP_URLS} <<DOCKER_HUB_ID>>/quarkusrobotshop-counter:latest
```

```shell
docker images -a | grep counter
docker tag <<RESULT>> <<DOCKER_HUB_ID>>/quarkusrobotshop-counter:<<VERSION>>
```
